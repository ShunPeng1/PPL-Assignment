    ...
    if len(argv) not in [1, 2]:
        ...
        if os.path.exists('./main/zcode2/parser'):
            subprocess.run(["java", "-jar", ANTLR_JAR, "-o", './main/zcode2/parser',
                        "-no-listener", "-visitor", "./main/zcode2/parser/ZCode.g4"])
        
        else:
            subprocess.run(["java", "-jar", ANTLR_JAR, "-o", './main/zcode/parser',
                        "-no-listener", "-visitor", "./main/zcode/parser/ZCode.g4"])
        
        if argv[1] in ['LexerSuite', 'ParserSuite']:
        ...
        if argv[1] == 'LexerSuite':
            subprocess.run('python genTestCase.py LexerSuite')

        elif argv[1] == 'ParserSuite':
            subprocess.run('python genTestCase.py ParserSuite')
