...
class Program(Visitor):
    ...
    def getExprList(self, mode, min, max):
        ...
        return "{}".format(
            ", ".join([self.visitExpression(mode) for i in range(random.randint(min, max))])
        )
    ...
    def visitVarDecl(self, mode):
        ...
        if mode == 'parser':
            ...
            if random.randint(0, 1) == 0:
                ...
                arr = "[{}]".format(
                    ",".join([self.visitNumberLit(mode) for i in range(len)])
                )
    ...
    def visitFuncDecl(self, mode):
        ...
        funcDecl = "func {} ({}){}{}{}{}".format(
            self.visitID(mode),
            ', '.join([x for x in [self.visitParamDecl(mode) for i in range(random.randint(0, 3))]]),
            has_newline,
            ''.join(tabarr),
            stmt,
            new_line_end
    ...
    def visitBlockStmt(self, mode):
        ...
        tabarr = ['\t'] * self.scope
        if mode == 'ast':
            block_stmt = "begin\n{}{}end".format(''.join([stmt for stmt in [''.join(tabarr) + self.getStmt(mode) + '\n' for i in range(random.randint(0, 3))]]), ''.join(tabarr[:-1]))

        else: 
            block_stmt = "begin\n{}{}end".format(
                ''.join([stmt for stmt in [f"{''.join(tabarr)}{self.getStmt(mode) if random.randint(0, 1) == 0 else self.visitComment(mode)}\n" for i in range(random.randint(0, 3))]]),
                ''.join(tabarr[:-1])
            )
    ...
    def visitReturnStmt(self, mode):
        return "return{}".format(" " + self.visitExpression(mode) if random.randint(0, 1) == 1 else "")
